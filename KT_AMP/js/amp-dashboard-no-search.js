...
document.addEventListener('DOMContentLoaded', function() {
    ...
    updateTabsForStep(currentStep);
        ...
        if (tabNav && !tabNav.innerHTML.trim()) {
            console.log('🚨 백업 탭 생성');
            tabNav.innerHTML = `
                <button class="tab-btn active" data-tab="fine-tuning" onclick="switchTab('fine-tuning')">
                    Fine Tuning 데이터
                </button>
                <button class="tab-btn" data-tab="biasing-code" onclick="switchTab('biasing-code')">
                    Biasing Code 데이터
                </button>
            `;
        }
...
// Auto-refresh functionality for monitoring tab
...
function startAutoRefresh() {
    ...
    // Start new interval - 30 seconds for general data refresh
    autoRefreshInterval = setInterval(() => {
        // Refresh current tab content
        if (currentTab) {
            loadTabContent(currentTab);
            console.log('🔄 자동 새로고침: 현재 탭 데이터 업데이트');
        }
    }, 30000); // 30 seconds
...
function updateRealTimeStats() {
    ...
    const stats = {
        learningProgress: Math.min(100, 73 + Math.random() * 5),
        modelAccuracy: Math.min(100, 94.2 + (Math.random() - 0.5) * 2),
        processingTime: Math.max(0.5, 1.8 + (Math.random() - 0.5) * 0.5),
        systemResource: Math.min(100, 68 + (Math.random() - 0.5) * 10)
    };
    
    // Update DOM elements
    const elements = {
        'learning-progress': stats.learningProgress.toFixed(0) + '%',
        'model-accuracy': stats.modelAccuracy.toFixed(1) + '%',
        'processing-time': stats.processingTime.toFixed(1) + 's',
        'system-resource': stats.systemResource.toFixed(0) + '%'
    };
...
function updateResourceMonitoring() {
    ...
    const resources = {
        cpu: Math.min(100, 72 + (Math.random() - 0.5) * 10),
        gpu: Math.min(100, 85 + (Math.random() - 0.5) * 8),
        memory: Math.min(100, 64 + (Math.random() - 0.5) * 12)
    };
    ...
    Object.entries(resources).forEach(([type, value]) => {
        ...
        if (usageElement) usageElement.textContent = value.toFixed(0) + '%';
        if (barElement) barElement.style.width = value.toFixed(0) + '%';
    });
}
...
function updateRecentActivities() {
    ...
    if (progressElement) {
        ...
        progressElement.textContent = newProgress.toFixed(0) + '%';
        
        const progressBar = progressElement.parentElement.querySelector('.progress-bar-fill');
        if (progressBar) progressBar.style.width = newProgress.toFixed(0) + '%';
    }
}

function updatePerformanceChart() {
    // This would update the chart.js chart
    // For now, we'll just ensure the canvas exists
...
function initializePerformanceChart() {
    ...
    const data = [70, 75, 73, 78, 82, 80, 85, 88, 91, 94];
    ...
    ctx.fillText('정확도 추이 (%)', canvas.width / 2, canvas.height - 5);
...
function updateNotifications() {
    ...
    // Check if we should add a new notification (random chance)
    if (Math.random() < 0.1) { // 10% chance
        const notifications = [
            {
                type: 'info',
                title: '학습 속도 향상',
                message: 'GPU 최적화로 학습 속도가 12% 향상되었습니다.',
                color: '#0ea5e9',
                bg: '#f0f9ff'
            },
            {
                type: 'success',
                title: '메모리 사용률 개선',
                message: '데이터 압축으로 메모리 효율성이 증가했습니다.',
                color: '#22c55e',
                bg: '#f0fdf4'
            },
            {
                type: 'warning',
                title: 'CPU 사용률 증가',
                message: 'CPU 사용률이 80%를 초과했습니다. 모니터링 중입니다.',
                color: '#f59e0b',
                bg: '#fffbeb'
            }
        ];
        ...
        notificationElement.className = 'alert-item';
        notificationElement.style.cssText = `
            display: flex; align-items: center; gap: 12px; padding: 12px; 
            background: ${randomNotification.bg}; border: 1px solid ${randomNotification.color}; 
            border-radius: 8px; margin-bottom: 8px;
        `;
        
        notificationElement.innerHTML = `
            <div style="width: 8px; height: 8px; background: ${randomNotification.color}; border-radius: 50%;"></div>
            <div>
                <div style="font-size: 14px; font-weight: 500;">${randomNotification.title}</div>
                <div style="font-size: 12px; color: #6b7280;">${randomNotification.message}</div>
            </div>
            <div style="font-size: 12px; color: #6b7280; margin-left: auto;">방금 전</div>
        `;
...
function updateTabsForStep(step) {
    ...
    console.log('✅ tabNavigation 요소 찾음:', tabNavigation);
    ...
    `).join('');
    console.log('✅ 탭 HTML 생성 완료:', tabNavigation.innerHTML);
